...
if user_file is not None:
	...
	if st.button("Columns Information"):
		x= data.columns
		a= list(x)
		s= data.sum(axis=0,skipna= True)
		st.write(s)
	...
	if st.button("Missing Values"):
		...
		for i in n:
			if i > 0:
					...
					data.isna().sum().reset_index(name="missing values").plot.bar(x='index', y='missing values', rot=45)
					st.pyplot()
					break
		...
		for i in cols_2:
			...
			for j in x1:
				if len(cols_3)== 0:
					...
					for k in cols_3:
						sns.scatterplot(x=modified[i],y=modified[j], hue= modified[k])
						st.pyplot()
...
@st.cache
	...
	if st.button('See the Plots for Analysis Report on a Sample Dataset'):
		data= pd.read_csv(dataset)
		st.write(data)

		
		st.header("Columns Information")
		x= data.columns
		a= list(x)
		s= data.sum(axis=0,skipna= True)
		st.write(s)
		...
		for i in cols_2:
			...
			for j in x1:
				if len(cols_3)== 0:
					...
					for k in cols_3:
						sns.scatterplot(x=new_data[i],y=new_data[j], hue= new_data[k])
						st.pyplot()
